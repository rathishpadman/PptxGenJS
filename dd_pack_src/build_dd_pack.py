...
from generic_common import *   # noqa: F401,F403

OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'dd_training_pack.pptx')
...
def header(kicker, title_runs, subtitle=None, *, dark=False):
    ...
    sub_col = SKY if dark else GREY_1
    if kicker:
        textbox(0.65, 0.36, 11.0, 0.26, [(kicker.upper(), 10.5, True,
                SKY if dark else TEAL)])
...
def slide_title():
    ...
    gradient(bg, NAVY_DEEP, NAVY, angle=35.0)
    shape(MSO_SHAPE.OVAL, 10.6, -1.7, 4.3, 4.3, fill=INDIGO)
    shape(MSO_SHAPE.OVAL, -1.4, 5.0, 3.4, 3.4, line=SKY, line_w=1.25)

    tag(0.9, 1.55, 3.55, 0.40, 'INTERNAL TRAINING PLAYBOOK', fill=INDIGO,
        txt_col=WHITE)
    textbox(0.85, 2.15, 11.3, 1.55,
            [[('Process Automation', 42, True, WHITE)],
             [('Due Diligence', 42, True, SKY)]], line_spacing=1.0)
...
def slide_agenda():
    ...
    header('Training roadmap', [('What we’ll cover ', BLACK),
                                ('today', TEAL)],
           'Seven sections, one worked example, one cheat sheet to keep.')
    ITEMS = [
        ('1', 'Why this matters', 'The cost of skipping DD, and what good DD buys you'),
        ('2', 'The DD lifecycle', 'Five stages, start to sign-off'),
    ...
    for i, (num, title, desc) in enumerate(ITEMS):
        ...
        chip(MSO_SHAPE.OVAL, 0.95, y + 0.13, 0.40, 0.40,
             [[(num, 13, True, WHITE)]], fill=NAVY if i < 5 else TEAL)
        textbox(1.55, y, 3.55, RH, [(title, 13, True, BLACK)],
                anchor=MSO_ANCHOR.MIDDLE)
        textbox(5.25, y, 7.20, RH, [(desc, 11, False, GREY_1)],
                anchor=MSO_ANCHOR.MIDDLE)
    footer(2, 'Agenda')
...
def slide_why():
    new_slide()
    header('Section 1', [('Why we do this ', BLACK), ('properly', TEAL)],
           'Skipped or shallow DD is the single biggest cause of automation '
           'write-offs.')
    CW, CT, CH = 5.75, 1.80, 4.60
    LX, RX = 0.65, 6.93

    card(LX, CT, CW, CH, fill=WHITE, line=RGBColor(0xF3, 0xC6, 0x8A))
    tag(LX, CT, 2.55, 0.42, 'IF YOU SKIP IT', fill=AMBER)
    RISKS = ['Bots built against SOPs nobody actually follows',
             'Exception rate discovered after go-live, not before',
             'Wrong technology picked — RPA thrown at a judgment call',
             'ROI case falls apart under finance scrutiny',
             'Security or compliance surprise late in delivery']
    for i, r in enumerate(RISKS):
        ry = CT + 0.66 + i * 0.72
        shape(MSO_SHAPE.OVAL, LX + 0.30, ry + 0.06, 0.18, 0.18, fill=AMBER)
        textbox(LX + 0.62, ry - 0.06, CW - 0.90, 0.62, [(r, 11.5, False, BLACK)],
                anchor=MSO_ANCHOR.MIDDLE, line_spacing=1.1)

    card(RX, CT, CW, CH, fill=WHITE, line=RGBColor(0x9F, 0xD9, 0xC9))
    tag(RX, CT, 2.55, 0.42, 'IF YOU DO IT RIGHT', fill=TEAL)
    GAINS = ['L4 ground truth — the process as it’s actually run',
             'Right-fit technology, not a default one',
             'Realistic FTE, cycle-time and error baselines',
             'A business case finance and ops both sign off on',
             'Executive confidence to fund Wave 2 and beyond']
    for i, gtext in enumerate(GAINS):
        ry = CT + 0.66 + i * 0.72
        shape(MSO_SHAPE.OVAL, RX + 0.30, ry + 0.06, 0.18, 0.18, fill=TEAL)
        textbox(RX + 0.62, ry - 0.06, CW - 0.90, 0.62, [(gtext, 11.5, False, BLACK)],
                anchor=MSO_ANCHOR.MIDDLE, line_spacing=1.1)
    footer(3, 'Why this matters')


# ═══════════════════════════════════════════════════ 4 · THE DD LIFECYCLE ═══
def slide_lifecycle():
    new_slide()
    header('Section 2', [('The DD lifecycle — ', BLACK), ('five stages', TEAL)],
    ...
    for i, (icon, title, desc) in enumerate(STAGES):
        x = X0 + i * (W + GAP)
        col = lerp((0x1E, 0x3A, 0x8A), (0x0D, 0x94, 0x88), i / (n - 1))
    ...
    footer(4, 'The DD lifecycle')


# ═══════════════════════════════════════════ 5 · BEFORE YOU WALK IN ═════════
def slide_prep():
    new_slide()
    header('Section 3', [('Before you walk in: ', BLACK), ('the prep checklist', TEAL)],
...
def facilitation_slide(n_slide, title_a, title_b, blocks_a, blocks_b, kicker, sub):
    new_slide()
    header(kicker, [(title_a, BLACK), (title_b, TEAL)], sub)
...
def slide_facilitation_1():
    facilitation_slide(
        6, 'Ask this: ', 'process & data',
        ('PROCESS & OPERATING LOGIC', NAVY,
        ...
        ('DATA & INPUT CHARACTERISTICS', TEAL,
...
def slide_facilitation_2():
    facilitation_slide(
        7, 'Ask this: ', 'systems & exceptions',
        ('SYSTEMS, ARCHITECTURE & ACCESS', NAVY,
        ...
        ('EXCEPTIONS, QUALITY & FALLBACK', TEAL,
...
def slide_redflags():
    new_slide()
    header('Section 3', [('Green flags, ', BLACK), ('red flags', TEAL)],
           'What good and bad answers sound like, across the four areas '
           'that matter most.')
    ROWS = [
        ('Volume', 'Documented, stable seasonality with real numbers',
         'Nobody can quote peak volume without guessing'),
        ('Business rules', 'Clear IF-THEN logic, exceptions are named',
         '“Depends who’s doing it” or “ask Priya”'),
        ('Systems access', 'Modern APIs confirmed, not just mentioned',
         'Screen-scrape only, Citrix, CAPTCHA, shared logins'),
        ('Roadmap stability', 'Systems stable for 12+ months, confirmed with IT',
         'ERP migration or redesign planned within 6 months'),
    ]
    TY = 1.86
    hx = 0.65
    shape(MSO_SHAPE.RECTANGLE, hx, TY, 12.03, 0.46, fill=NAVY)
    for cx, cw, lab in ((hx, 2.35, 'AREA'), (hx + 2.35, 4.84, 'GREEN FLAG — PROCEED'),
                        (hx + 7.19, 4.84, 'RED FLAG — DIG DEEPER')):
        textbox(cx + 0.16, TY, cw - 0.2, 0.46, [(lab, 10, True, WHITE)],
                anchor=MSO_ANCHOR.MIDDLE)
    for i, (area, good, bad) in enumerate(ROWS):
        ry = TY + 0.46 + i * 0.98
        if i % 2 == 0:
            shape(MSO_SHAPE.RECTANGLE, hx, ry, 12.03, 0.98, fill=GREY_4)
        textbox(hx + 0.16, ry, 2.19, 0.98, [(area, 12, True, BLACK)],
                anchor=MSO_ANCHOR.MIDDLE)
        shape(MSO_SHAPE.OVAL, hx + 2.35 + 0.14, ry + 0.34, 0.24, 0.24, fill=TEAL)
        textbox(hx + 2.35 + 0.50, ry, 4.20, 0.98, [(good, 10.5, False, BLACK)],
                anchor=MSO_ANCHOR.MIDDLE, line_spacing=1.1)
        shape(MSO_SHAPE.OVAL, hx + 7.19 + 0.14, ry + 0.34, 0.24, 0.24, fill=AMBER)
        textbox(hx + 7.19 + 0.50, ry, 4.20, 0.98, [(bad, 10.5, False, BLACK)],
                anchor=MSO_ANCHOR.MIDDLE, line_spacing=1.1)
    footer(8, 'Running the workshop')
...
def slide_complexity():
    new_slide()
    header('Section 4', [('Scoring complexity — ', BLACK), ('three dimensions', TEAL)],
    ...
    for i, (wt, name, lo, hi, frac) in enumerate(DIMS):
        y = CT + i * (CH + GAP)
        col = lerp((0x1E, 0x3A, 0x8A), (0x0D, 0x94, 0x88), i / 2)
...
def slide_impact():
    new_slide()
    header('Section 4', [('Scoring impact — ', BLACK), ('two dimensions', TEAL)],
...
def slide_quadrant():
    new_slide()
    header('Section 4', [('Where it lands: ', BLACK), ('the prioritization quadrant', TEAL)],
           'Plot the two scores — impact on the vertical axis, complexity '
           'on the horizontal.')
    GX, GY, GW, GH = 2.10, 1.90, 8.6, 4.6
    shape(MSO_SHAPE.RECTANGLE, GX, GY, GW / 2, GH / 2, fill=ICE_TEAL)
    ...
    shape(MSO_SHAPE.RECTANGLE, GX + GW / 2, GY + GH / 2, GW / 2, GH / 2,
          fill=RGBColor(0xFB, 0xE4, 0xD5))
    card(GX, GY, GW, GH, fill=None, line=GREY_2)
    vrule(GX + GW / 2, GY, GH, GREY_2, 1.0)
    rule(GX, GY + GH / 2, GW, GREY_2, 1.0)

    QUADS = [
        (GX + 0.14, GY + 0.14, 'QUICK WINS', TEAL_DEEP, 'Priority Wave 1 · high ROI, fast delivery'),
        (GX + GW / 2 + 0.14, GY + 0.14, 'STRATEGIC BETS', NAVY, 'Priority Wave 2 · high value, needs AI/architecture'),
        ...
        (GX + GW / 2 + 0.14, GY + GH / 2 + 0.14, 'DEPRIORITIZE / RE-ENGINEER',
         RGBColor(0xB4, 0x53, 0x09), 'Do NOT automate as-is — fix the process first'),
    ]
...
def slide_tech():
    new_slide()
    header('Section 5', [('Choosing the ', BLACK), ('technology', TEAL)],
           'Five archetypes cover almost everything you’ll see. Route the '
           'candidate to the lever that matches how it actually behaves.')
    ROWS = [
        ('gear', 'RPA', NAVY, 'Structured data, deterministic rules, repetitive UI keystrokes',
         'Order entry, data migration, screen-to-screen re-keying'),
        ('doc', 'IDP', TEAL, 'Semi-structured documents that need extraction',
         'Invoices, remittances, claims forms, IDs'),
        ('bulb', 'AI-Enabled Automation', NAVY,
    ...
    shape(MSO_SHAPE.RECTANGLE, hx, TY, 12.03, 0.40, fill=NAVY_DEEP)
    ...
    shape(MSO_SHAPE.ROUNDED_RECTANGLE, hx, fy, 12.03, 0.52, fill=RGBColor(0xFB, 0xE4, 0xD5),
          adj=0.2)
    shape(MSO_SHAPE.OVAL, hx + 0.16, fy + 0.10, 0.32, 0.32, fill=AMBER)
    glyph('target', hx + 0.32, fy + 0.26, 0.18, WHITE)
    textbox(hx + 0.62, fy, 11.2, 0.52,
            [[('Process broken beyond repair? ', 11, True, RGBColor(0xB4, 0x53, 0x09)),
...
def slide_mechanics():
    new_slide()
    header('Section 6', [('Business case ', BLACK), ('mechanics', TEAL)],
    ...
    tag(0.90, 2.02, 2.6, 0.36, 'BENEFIT FORMULA', fill=NAVY)
    ...
    card(0.65, CT2, CW2, CH2, fill=ICE_BLUE, line=NAVY)
    tag(0.90, CT2 + 0.20, 2.9, 0.36, 'ONE-TIME BUILD COST', fill=NAVY)
    ...
    card(6.78, CT2, CW2, CH2, fill=ICE_TEAL, line=TEAL)
    tag(7.03, CT2 + 0.20, 2.9, 0.36, 'ANNUAL RUN COST', fill=TEAL)
...
def wx_header(n, title_b, sub):
    header('Section 7 · Worked example', [('Order-to-Cash: ', BLACK), (title_b, TEAL)], sub)
...
def slide_wx_process():
    ...
    for i, (val, lab) in enumerate(STATS):
        sx = X0 + i * sw
        textbox(sx, SY + 0.14, sw, 0.42, [(val, 22, True, NAVY)],
                align=PP_ALIGN.CENTER)
...
def slide_wx_scored():
    ...
    tag(0.90, 2.00, 2.9, 0.34, 'COMPLEXITY  ·  WEIGHTED 2.70', fill=NAVY, size=10)
    CDIMS = [('Data & Input (35%)', 3, NAVY), ('Process Logic (35%)', 3, NAVY),
             ('Technical & System (30%)', 2, TEAL)]
    ...
    tag(7.08, 2.00, 2.9, 0.34, 'IMPACT  ·  WEIGHTED 4.50', fill=TEAL, size=10)
    IDIMS = [('Productivity (50%)', 5, NAVY), ('Business Outcome (50%)', 4, TEAL)]
    ...
    tag(7.08, 5.30, 4.3, 0.42, 'PLACEMENT → QUICK WIN', fill=TEAL_DEEP, size=11)
...
def slide_wx_roi():
    ...
    ARCH = [('doc', 'IDP', TEAL, 'Extract remittance data from PDFs / scans'),
            ('bulb', 'AI-Enabled', NAVY, 'Fuzzy-match remittance lines to invoices'),
            ('union', 'Agentic', TEAL, 'Triage the 22% exceptions, recommend action'),
            ('gear', 'RPA', NAVY, 'Post matched cash into SAP')]
    CW, CT = 2.85, 1.80
    ...
    card(0.65, 3.66, 12.03, 2.72, fill=ICE_BLUE, line=NAVY)
    tag(0.90, 3.86, 3.0, 0.36, 'ILLUSTRATIVE ROI SUMMARY', fill=NAVY)
    METRICS = [('$95K', 'Build cost'), ('$150K', 'Year-1 net benefit'),
               ('~8 mo', 'Payback from go-live'), ('$390K', '3-yr cumulative net value')]
    mw = 11.5 / 4
    for i, (val, lab) in enumerate(METRICS):
        mx = 0.90 + i * mw
        textbox(mx, 4.36, mw - 0.2, 0.62, [(val, 28, True, NAVY)])
    ...
    textbox(0.90, 5.62, 11.5, 0.60,
            [('Numbers are illustrative, built from the formulas in Section 6, '
              'to show the shape of the case — not a substitute for the live '
              'workshop model.', 9.5, False, GREY_1)], line_spacing=1.15)


# ═══════════════════ 17 · SIGN-OFF & EXEC PRESENTATION ══════════════════════
def slide_signoff():
    new_slide()
    header('Section 8', [('Getting to ', BLACK), ('sign-off', TEAL)],
           'Three internal gates before the room ever sees an executive '
           'deck.')
    card(0.65, 1.82, 5.75, 4.55)
    tag(0.90, 2.02, 3.2, 0.36, 'THREE STAGE GATES', fill=NAVY)
    ...
    card(6.83, 1.82, 5.85, 4.55, fill=ICE_TEAL, line=TEAL)
    tag(7.08, 2.02, 3.5, 0.36, 'EXECUTIVE DECK STRUCTURE', fill=TEAL)
    ...
    for i, t in enumerate(PARTS):
        py = 2.58 + i * 0.56
        textbox(7.08, py, 0.32, 0.30, [(str(i + 1) + '.', 10.5, True, TEAL)])
...
def slide_cheatsheet():
    new_slide()
    header('Keep this at your desk', [('The one-page ', BLACK), ('cheat sheet', TEAL)],
           'Everything above, compressed for the field.')

    card(0.65, 1.78, 3.85, 4.60)
    tag(0.90, 1.98, 2.2, 0.32, 'LIFECYCLE', fill=NAVY, size=9)
    ...
    tag(0.90, 4.80, 2.2, 0.32, 'SCORING', fill=TEAL, size=9)
    ...
    tag(4.93, 1.98, 2.5, 0.32, 'TECHNOLOGY ARCHETYPES', fill=NAVY, size=9)
    for i, (icon, t) in enumerate([('gear', 'RPA'), ('doc', 'IDP'),
                                   ('bulb', 'AI-Enabled Automation'),
                                   ('union', 'Agentic'), ('layers', 'BPM')]):
        ty = 2.46 + i * 0.60
        shape(MSO_SHAPE.OVAL, 4.93, ty, 0.36, 0.36, fill=TEAL)
    ...
    textbox(4.93, 5.68, 3.35, 0.60,
            [('Broken process? Fix it first — don’t automate chaos.',
              9.5, True, AMBER)], line_spacing=1.15)

    card(8.71, 1.78, 3.97, 4.60, fill=ICE_BLUE, line=NAVY)
    tag(8.96, 1.98, 2.6, 0.32, 'THE FORMULA', fill=NAVY, size=9)
    ...
    tag(8.96, 3.98, 2.6, 0.32, 'GREEN VS RED FLAGS', fill=TEAL, size=9)
    textbox(8.96, 4.36, 3.55, 0.42,
            [[('Green: ', 10, True, TEAL), ('APIs confirmed, stable roadmap',
               9.5, False, BLACK)]], line_spacing=1.1)
    textbox(8.96, 4.72, 3.55, 0.42,
            [[('Red: ', 10, True, AMBER), ('“ask Priya”, migration in 6 months',
               9.5, False, BLACK)]], line_spacing=1.1)
    footer(18, 'Cheat sheet')


for fn in (slide_title, slide_agenda, slide_why, slide_lifecycle, slide_prep,
           slide_facilitation_1, slide_facilitation_2, slide_redflags,
           slide_complexity, slide_impact, slide_quadrant, slide_tech,
           slide_mechanics, slide_wx_process, slide_wx_scored, slide_wx_roi,
           slide_signoff, slide_cheatsheet):
    fn()
